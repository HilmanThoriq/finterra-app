// src/screens/main/EditExpenseModal.js
...
export default function EditExpenseModal({ visible, onClose, expense, onUpdate }) {
  ...
  useEffect(() => {
    if (visible && expense) {
      setAmount(expense.amount?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') || '');
